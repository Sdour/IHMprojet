...
function initScene(scene) {
    ...
    answer.each(function() {
        ...
        if(!el.data('correct')) {
            ...
            while(hna == videoGames[rand]){
                rand = Math.floor(Math.random() * (videoGames.length - 1));
            }
...
$(answer).click(function() {
    ...
    if(!transitioning) {
        ...
        if(checkAnswer($(this))) {
            ...
            // Play sound
            playSound(featured);
            playSound(scenes[progress - 1].hint);
            ...
            $('.main_inner__feedback').text(scenes[progress - 1].answer).addClass('correct');
            ...
            $('.main_inner__feedback').text(scenes[progress - 1].answer).addClass('wrong');
            $('.main_inner__feedback').css('transform', 'translateY(-50%) scale(1) rotate(0deg)');
        ...
        if(aide == $('.scene').length) {
            ...
            if(score >= 40){
            	$('h1.testScore').html('Excellent !');
            }else if(score >= 25){
            	$('h1.testScore').html('Bien !');
            }else{
            	$('h1.testScore').html('Tu peux faire mieux !');
            	$('button.playAgain').css('margin-top' , '+160px');
                $('button.playAgainn').css('margin-top' , '+160px');
            }
            $('p.score').html('Votre score est :  ' + score + ' points.');
            //writeToFile(score);
            $.ajax({
